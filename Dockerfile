...
# Compatibility-first template for circlator.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/circlator" ]; then BIN="/opt/conda/bin/circlator"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo circlator | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'circlator*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/circlator
RUN chmod +x /usr/local/bin/circlator && rm -f /tmp/tool-entry-path
WORKDIR /data
ENTRYPOINT ["/usr/local/bin/circlator"]
